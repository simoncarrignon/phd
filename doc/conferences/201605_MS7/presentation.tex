...
\documentclass[12pt, notes=show,handout=no]{beamer}
...
\begin{frame}{Evolutionary biology toolkit}
    ...
    \vfill
    	\begin{itemize}
	    \item Computer Simulation to study complex mathematical model
    \vfill
	    \item Computer Model (Agent Based, Cellular Automata\dots) to describe complex systems
	\end{itemize}
    \vfill
...
\begin{frame}{Model Selection}
    ...
    \end{center}
    \uncover<2->Four diff. models to explain Battle Casualties
\end{frame}
...
\begin{frame}{Model Selection}
    ...
    \end{itemize}
    \begin{enumerate}
	\item<1->Linear (one to one fights, size$++$)
	\item<2->Squared (one to many fights techno$++$)
	\item<3->Logarithmic (one to many fights techno$++$)
	\item<4->Fatigue (\dots \& decrease in time)
    \end{enumerate}
	...
	\begin{tabular}{cccc}
	    ...
	    \uncover<5->{\tiny let $P=  r/b = 1.5,B_0-15000,R_0=1000$}\\
