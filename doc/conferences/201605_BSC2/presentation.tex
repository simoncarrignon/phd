...
\begin{frame}{Cultural Evolution}
	...
	\begin{table}
	    \center
	    \begin{tabular}{ccc}
		\uncover<1->{\includegraphics[height=3cm]{images/m80}} &
		\uncover<2->{\includegraphics[height=3cm]{images/m90}} &
		\uncover<3->{\includegraphics[height=3cm]{images/m10}} \\
		\uncover<1->{80's} & \uncover<2->{90's} & \uncover<3->{now}
	    \end{tabular}
	\end{table}
    \end{center}
    \uncover<4->{How Social Traits Evolve?}\uncover<5>{ Cultural Evolution }
\end{frame}
...
\begin{frame}{Cultural Evolution}
    \uncover<1->{Social traits:}
    \begin{itemize}
	\item<5->{ culturally transmitted, socially learnt}
	\item<6->{ similar patterns}
    \end{itemize}
	\begin{center}
		\uncover<2->{\includegraphics[width=3cm]{images/cutdog}}\\
		\vspace{.5cm}
		\uncover<4->{\includegraphics[width=2.5cm]{images/cutbaby}}
	    \hspace{1cm}
...
%    Available database, twitter, photo\dots
%
%    $\rightarrow$ Comparison between cultures and across time need : Archaeology.
%
%    
%\end{frame}
