...
\begin{frame}{Another approach}
    \huge
    \centering
    Integrate social-cultural mechanism to those economic interactions.
...
\begin{frame}{What Generates Those Cultural changes?}
	...
	\begin{itemize}
	    \item<2->Individual learning: I learn by my own error or I suddenly have a new idea (\emph{ie} ``innovation''). 
		\item<3-> Social learning : I learn by exchanging with the other or by watching them. 
	\end{itemize}
\end{frame}
	...
	\uncover<2->{\begin{figure}
		\begin{columns}
			\begin{column}{.7\textwidth}
				\centering
				\uncover<4->{	\includegraphics[width=1\textwidth]{images/steeletal2010Distrib.jpg}}
			\end{column}
			\begin{column}{.3\textwidth}
			    \uncover<3->{\includegraphics[width=1\textwidth]{images/steeletal2010rim}\\\vspace{.5cm}
...
\begin{frame}{``Neutral'' model }
	...
	\item negligible economic cost 
	    ...
	    \end{itemize}
	\item no preferences (prestige\dots)
    \end{itemize}
...
\begin{frame}
	\begin{center}
	    What if such mechanisms act on traits explicitly linked to economics?
	\end{center}
\end{frame}
...
\begin{frame}{Specification of the model}
    \begin{itemize}
	\item Integrate historical knowledge: \\
	$\rightarrow$ Calibrate the model to produce more ``realistic'' simulations
	    \begin{itemize}
		\item 5 different types of good introduce one after this other
...
\begin{frame}{Individual vs Social learning}
    ...
    \begin{itemize}
	\item<2-> agents are markets,
	\item<3-> \textbf{individual} learning (innovation): \\
	    \uncover<4->{ \hspace{.5cm} $\rightarrow$ the capacity of a market to generate new solutions by itself (internal changes, interactions within a market: \emph{ie} \textbf{creativity}) } 
	\item<5-> \textbf{social} learning (cultural transfer): \\
	    \uncover<6->{ \hspace{.5cm} $\rightarrow$ the tendency of a market to copy another (better) one (interactions, communication between markets: \emph{ie} \textbf{openness}) }
    \end{itemize}
\end{frame}
...
Individual  vs Social learning  ( $\mu$ vs $\lambda$ )
    \begin{itemize}
	\item $\mu \rightarrow$ increase individual learning (``a settlement decides to change the value of a ware'')\\
	    \hspace{.2cm} $\mu \in \{0.001,0.01,0.1,1\}$
	\item $\lambda\rightarrow$ increase the proba. to copy a more successful settlement\\
