...
cat $1  | awk 'BEGIN{FS=";"}{if($5>1904 && !($2 ~ /eomet/) ) print $14}'  | grep pdf | sed 's/pdf/txt/' > unlab #tex newer than 1904
cat $1  | awk 'BEGIN{FS=";"}{if($5>1904 && ($2 ~ /eomet/) ) print $14}'  | grep pdf | sed 's/pdf/txt/' > geom #tex newer than 1904
